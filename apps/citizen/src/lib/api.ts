import type {
  ...
  Dispatch, SituationBrief, EventStats, Event,
  IncidentStatus,
} from '@/types'
...
export async function resolveDispatch(dispatch_id: string, notes?: string): Promise<void> {
  return request(`/dispatch/${dispatch_id}/resolve?notes=${encodeURIComponent(notes || '')}`, {
    method: 'PATCH',
  })
}
...
export async function transcribeAudio(blob: Blob, filename = 'recording.webm'): Promise<{ transcript: string; language?: string }> {
  const form = new FormData()
  form.append('audio', blob, filename)
  const res = await fetch(`${BASE_URL}/ai/transcribe`, { method: 'POST', body: form })
